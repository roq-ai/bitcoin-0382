...
import { getMinings } from 'apiSdk/minings';
...
import { ProfitListPage as ShowCaseList } from 'pages/profits';

function DashboardPage() {
  const { session } = useSession();

  const miningStat = useSWR<PaginatedInterface<MiningInterface>>(
    () => `/minings`,
    () => getMinings({ limit: 1, offset: 0 }),
  );

  const depositStat = useSWR<PaginatedInterface<DepositInterface>>(
    () => `/deposits`,
    () => getDeposits({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Minings',
      result: miningStat?.data?.totalCount,
      isLoading: miningStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Deposits',
      result: depositStat?.data?.totalCount,
      isLoading: depositStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Profits',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`profits`} passHref legacyBehavior>
                  <Button
