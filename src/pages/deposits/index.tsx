...
import { getDeposits, deleteDepositById } from 'apiSdk/deposits';
import { DepositInterface } from 'interfaces/deposit';

type ColumnType = ColumnDef<DepositInterface, unknown>;
...
export function DepositListPage(props: DepositListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<DepositInterface>>(
    () => `/deposits?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: DepositInterface) => {
    if (hasAccess('deposit', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/deposits/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/deposits/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('deposit', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/deposits/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('deposit', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="deposit" />
        </Flex>

        {hasAccess('deposit', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/deposits/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
